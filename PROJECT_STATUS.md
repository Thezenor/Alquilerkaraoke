...
## Conversión — testimonios y pack destacado (2026-06-10)
- [x] **Testimonios (prueba social)**: modelo `Testimonial` (autor, tipo de evento, cita, rating 1–5, idioma es/en/fr, enlace a la reseña, activo/orden). Migración `testimonials_and_featured_pack`. **Admin** `/admin/testimonios` (CRUD, rol SUPERADMIN/ADMIN, Zod, auditoría, `updateTag`) + entrada en navegación (Catálogo). **Público**: sección "Lo que dicen nuestros clientes" en la home (estrellas SVG accesibles, 1 col móvil → 3 desktop, solo activos del idioma con fallback a ES; oculta si no hay datos) + **JSON-LD `aggregateRating` + `Review`** (media con 1 decimal + 3 reseñas) añadido al `Service` de la home **solo con datos reales en BD**. `src/server/testimonials.ts` cacheado por tag. Sin seed (testimonios siempre reales desde admin).
- [x] **Pack destacado ("Más popular")**: `Pack.isFeatured` (misma migración) + checkbox "Destacado (más popular)" en el admin de packs (badge en el listado). En `/packs` el destacado lleva **badge pill i18n + borde cian con glow**. No se fuerza unicidad en BD (si hay varios, todos llevan badge).

## Rendimiento móvil (auditoría CWV) — 2026-06-10
- [x] **next/image en toda la web pública**: componente `SmartImage` (`src/components/site/smart-image.tsx`, fill + `sizes` reales; URL externa → `<img>`; SVG → sin optimizar) en cards de packs/blog/eventos/galerías, heros de detalle (`priority` = LCP), imágenes de Markdown (wrapper 16/9 + `object-contain` → sin CLS) y logos de colaboradores (width/height). `next.config.ts`: `images.formats` AVIF/WebP. Verificado `/_next/image?url=/media/...` → 200 `image/avif`.
- [x] **ISR en fichas SEO** (patrón `karaoke/[ciudad]`: `revalidate=3600` + `generateStaticParams → []`, sin `force-dynamic`): `servicios/[slug]`, `packs/[slug]`, `eventos/[slug]`, `blog/[slug]`. `galerias/[slug]` NO migrada (lee cookies para la clave). Listados siguen `force-dynamic`. Verificado `x-nextjs-cache: MISS→HIT`.
- [x] **/canciones**: `getCatalogStats` cacheada sustituye al count directo; `getLanguageCounts` ahora `unstable_cache` (tag `SONGS_TAG`, lo invalida el import del admin).
- [x] **Índices de canciones** (migración `song_search_indexes`): GIN trigram sobre `title`/`performer` (sirven al ILIKE de Prisma; se eliminan los antiguos sobre `lower(col)` que el planner no usaba) + B-tree `(isPrimary, title)` y `(isPrimary, performer)`.
- [x] **Detalles**: layout con `Promise.all`; mensajes cliente acotados con pick de namespaces (−~25 KB por página; si añades `useTranslations` en un componente cliente nuevo, añade su namespace en `[locale]/layout.tsx`); blog listado con select sin `content`; galería bloqueada/caducada ya no carga items; `<video>` con `preload="none"`; `/media` con ETag/Last-Modified + 304; Cache-Control para assets estáticos propios (svg/iconos//packs).
...
- [x] **Subida desde el admin + exportaciones** (2026-06-08): subida del catálogo **.xlsx o .csv hasta ~200 MB** desde `/admin/canciones` mediante un **route handler** que recibe el fichero por streaming a un temporal y lo procesa **en segundo plano** (`SongImportJob` con progreso en vivo que el panel consulta). Lectura por streaming con **exceljs** (xlsx) y **csv-parse** (csv) → inserción por lotes + optimización; reemplaza el catálogo. **Exportar CSV** (streaming, admin) y **PDF del repertorio por idioma** (`/canciones/pdf`, 2 columnas, tope 8.000). Migración `song_import_job`. E2E `canciones-import` (subir→publicar→PDF→CSV→vaciar). Validado el parser xlsx y la deduplicación.
...
## Extras admin
- [x] **Acceso al panel desde el menú público** ("Acceso" en cabecera + móvil → `/admin/login`). Restaurado en el header (desktop + panel móvil) a petición del cliente (2026-06-11); se mantiene también el enlace del footer.
...
- [x] **Admin de usuarios mejorado + Mi cuenta** (2026-06-11): campo `User.lastLoginAt` (migración `user_last_login`, se actualiza en el evento `signIn`, best-effort). Lista con badges de rol por color (`RoleBadges`), estado, alta y último acceso + contador. Salvaguarda del **último superadmin activo** (no se puede desactivar, quitarle el rol ni eliminarlo). **Eliminar usuario** (`deleteUser`, solo SUPERADMIN, ConfirmButton + auditoría `user.delete`): borrado **físico** — decisión: todas las FKs hacia `User` son `SetNull` (AuditLog, reservas, pagos, contratos, posts) o `Cascade` (accounts/sessions), la auditoría se conserva anónima y el email/roles del borrado quedan en el metadata del log. Edición con panel de metadatos + últimas 10 acciones de auditoría. Descripciones de permisos reales bajo cada rol en el formulario. **`/admin/perfil` (Mi cuenta, todos los roles del panel)**: nombre editable, email solo lectura, roles, último acceso y cambio de contraseña propio verificando la actual (`user.self_update` / `user.password_change`); enlazado desde el email del shell y como ítem "Mi cuenta" del nav. Verificado: typecheck + lint + unit 65/65 + build + E2E usuarios + curl (header con `/admin/login` ×2 y `lastLoginAt` actualizado en login real).
